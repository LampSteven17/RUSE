...
def make_smol_step_callback(logger: "AgentLogger"):
    ...
    def _callback(memory_step, **_kwargs):
        ...
        # Parse errors can have no code_action. Record them without introducing
        # execution policy: the framework owns step limits and interruption.
        if _is_smol_parse_error(err):
            logger.step_error("code_parse", message=str(err)[:200])
            return
