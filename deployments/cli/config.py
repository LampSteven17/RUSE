...
class DeploymentConfig:
    ...
    deployment_name: str
    deployment_type: str = "sup"  # "sup", "rampart", or "ghosts"
    ...
    def load(cls, config_path: Path) -> DeploymentConfig:
        ...
        return cls(
            ...
            deployment_type=raw.get("type", "sup"),
    ...
    def expand_vms(self, dep_id: str) -> list[dict]:
        """Expand deployments into flat VM list with names."""
        vms = []
        for d in self.deployments:
            ...
            for i in range(count):
                name_behavior = behavior.replace(".", "-")
                vm_name = f"r-{dep_id}-{name_behavior}-{i}"
                vms.append({
                    "name": vm_name,
                    "behavior": behavior,
                    "flavor": flavor,
