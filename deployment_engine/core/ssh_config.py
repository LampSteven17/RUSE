"""SSH config block management for deployments managed by the RUSE CLI."""
...
def remove_all_managed_blocks(ssh_config: Path | None = None) -> list[str]:
    """Remove all managed SSH config blocks. Returns list of removed deployment names."""
    ssh_config = ssh_config or DEFAULT_SSH_CONFIG
    if not ssh_config.exists():
        return []
